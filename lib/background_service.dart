...
class BackgroundService {
  ...
  Future<void> executeTask() async {
    ...
    _logger.i("BackgroundService: Found ${upcomingDoses.length} upcoming doses.");

    for (final dose in upcomingDoses) {
      _logger.i("BackgroundService: Processing dose: ${dose.medicationName} at ${dose.time}, status: ${dose.status}, notificationSentCount: ${dose.notificationSentCount}");
      if (dose.status == DoseStatus.expired && dose.notificationSentCount < 4) {
        _logger.i("BackgroundService: Sending notification for expired dose: ${dose.medicationName}");
        ...
        _logger.i("BackgroundService: Notification sent and dose updated for ${dose.medicationName}. New count: ${updatedDose.notificationSentCount}");
      } else if (dose.status == DoseStatus.expired && dose.notificationSentCount >= 4) {
        _logger.w("BackgroundService: Max notifications sent for ${dose.medicationName}. Skipping.");
      }
    }
